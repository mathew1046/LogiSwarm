...
class RouteOptimizationEngine:
    """Compute top-3 route alternatives using a disruption-aware weighted graph."""
    ...
    def _k_shortest_paths(self, *, origin: str, destination: str, blocked: set[str], k: int) -> list[RouteAlternative]:
        candidates: list[RouteAlternative] = []

        # Multi-path best-first search with path-state to approximate top-k candidates.
        queue: list[tuple[float, str, list[str], float, float, float]] = [(0.0, origin, [origin], 0.0, 0.0, 1.0)]
        ...
        while queue and len(candidates) < k:
            ...
            for edge in self.graph.get(node, []):
                ...
                heapq.heappush(
                    queue,
                    (weighted_score, target, next_path, next_cost, next_eta, next_reliability),
                )

        return candidates

    async def _summarize_with_llm(self, *, current_route: list[str], result: RouteOptimizationResult) -> str:
    ...
    @staticmethod
    def _fallback_summary(*, current_route: list[str], result: RouteOptimizationResult) -> str:
    ...
    def _default_route_graph() -> dict[str, list[dict[str, Any]]]:
        return {
            "china_ea": [
                ...
                {"target": "north_america", "cost": 2.8, "time": 28.0, "reliability": 0.84},
            ],
            "se_asia": [
                {"target": "gulf_suez", "cost": 1.2, "time": 18.0, "reliability": 0.9},
                {"target": "north_america", "cost": 3.3, "time": 30.0, "reliability": 0.82},
            ],
            "gulf_suez": [
                {"target": "europe", "cost": 1.1, "time": 14.0, "reliability": 0.91},
                {"target": "north_america", "cost": 2.6, "time": 23.0, "reliability": 0.8},
            ],
            "europe": [
                {"target": "north_america", "cost": 1.7, "time": 12.0, "reliability": 0.89},
